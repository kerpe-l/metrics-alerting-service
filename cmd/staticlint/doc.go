...
// два публичных сторонних анализатора и собственный анализатор osexit.
// Используется как локально, так и в CI поверх всего модуля.
//
// # Запуск
//
// Сборка и запуск:
...
//
// 5. Собственный анализатор osexit (см. подпакет ./osexit) — запрещает прямой
// вызов os.Exit в функции main пакета main. Импорт os под алиасом учитывается:
// детект идёт через type info, а не по тексту.
package main
