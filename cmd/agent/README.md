...
В каждый запрос агент проставляет свой исходящий IP-адрес в заголовке `X-Real-IP`. Если на сервере задана доверенная подсеть (`-t` / `TRUSTED_SUBNET`), запросы с IP вне подсети отклоняются с кодом `403 Forbidden`.
